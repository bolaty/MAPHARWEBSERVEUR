...
namespace Stock.WSDAL
{
	...
	public class clsJourneetravailWSDAL : ITableDAL<clsJourneetravail>
	{
        ...
        ///<returns>Un string comme valeur du résultat de la requete</returns>
        ///<author>Home Technologie</author>
        public string pvgValueScalarRequeteMaxDateActive(clsDonnee clsDonnee , params string[] vppCritere)
        {
	        pvpChoixCritere1(vppCritere);
	        this.vapRequete = "SELECT MAX(JT_DATEJOURNEETRAVAIL) AS JT_DATEJOURNEETRAVAIL  FROM JOURNEETRAVAIL "+ this.vapCritere ;
            this.vapCritere = "";
        ...
        ///<returns>DataSet</returns>
        ///<author>Home Technologie</author>
        public DataSet pvgChargerDansDataSetJourneeEncours(clsDonnee clsDonnee, params string[] vppCritere)
        {
            vapNomParametre = new string[] { "@AG_CODEAGENCE" , "@EX_EXERCICE" };
            vapValeurParametre = new object[] { vppCritere[0], vppCritere[1] };
            this.vapRequete = "SELECT  TOP 1 * FROM  dbo.JOURNEETRAVAIL GROUP BY AG_CODEAGENCE, JT_DATEJOURNEETRAVAIL ,JT_STATUT,OP_CODEOPERATEUR HAVING AG_CODEAGENCE=@AG_CODEAGENCE AND  year(JT_DATEJOURNEETRAVAIL)=@EX_EXERCICE ORDER BY  JT_DATEJOURNEETRAVAIL DESC";
            this.vapCritere = "";
            SqlCommand vppSqlCmd = new SqlCommand(this.vapRequete, clsDonnee.vogObjetConnexionLocal, clsDonnee.vogObjetTransactionLocal);
            return clsDonnee.pvgRemplirDataset(vppSqlCmd, vapNomParametre, vapValeurParametre, true);
        }
