MapharApp.controller("TransAuxiliaireController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisAuxiliaire = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT SANTE AUXILIAIRE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransAuxiliaire", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpTransAuxiliaire")
      );
    };
    //fin voir plus
    ...
    $scope.transmissionAUX = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisAuxiliaire = reponse.data;
          if (
            $scope.elmntTransmisAuxiliaire[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisAuxiliaire[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
