MapharApp.controller("TransMultirisqueProController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisMultirisquePro = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT MULTIRISQUE PROFESSIONNEL";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransMultirisquePro", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpTransMultirisquePro")
      );
    };
    //fin voir plus
    ...
    $scope.transmissionMP = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisMultirisquePro = reponse.data;
          if (
            $scope.elmntTransmisMultirisquePro[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisMultirisquePro[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
