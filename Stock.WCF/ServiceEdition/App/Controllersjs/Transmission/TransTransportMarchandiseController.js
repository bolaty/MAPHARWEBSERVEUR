MapharApp.controller("TransTransportMarchandiseController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisTranspMarch = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT TRANSPORT MARCHANDISE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransTranspMarch", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpTransTranspMarch")
      );
    };
    //fin voir plus
    ...
    $scope.transmissionTM = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisTranspMarch = reponse.data;
          if (
            $scope.elmntTransmisTranspMarch[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisTranspMarch[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
