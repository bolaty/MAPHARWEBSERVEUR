MapharApp.controller("TransVoyageController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisVoyage = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT VOYAGE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransVoyage", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpTransVoyage"));
    };
    //fin voir plus
    ...
    $scope.transmissionVoyage = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisVoyage = reponse.data;
          if (
            $scope.elmntTransmisVoyage[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisVoyage[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
