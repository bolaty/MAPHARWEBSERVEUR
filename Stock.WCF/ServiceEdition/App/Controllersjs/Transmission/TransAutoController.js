MapharApp.controller("TransAutoController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisAuto = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT AUTO";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransAuto", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpTransAuto"));
    };
    //fin voir plus
    ...
    $scope.transmissionAuto = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisAuto = reponse.data;
          if (
            $scope.elmntTransmisAuto[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisAuto[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
