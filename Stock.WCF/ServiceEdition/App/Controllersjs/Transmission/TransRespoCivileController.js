MapharApp.controller("TransRespoCivileController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisRespoCivile = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT RESPONSABILITE CIVILE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransRespoCivile", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpTransRespoCivile")
      );
    };
    //fin voir plus
    ...
    $scope.transmissionRC = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisRespoCivile = reponse.data;
          if (
            $scope.elmntTransmisRespoCivile[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisRespoCivile[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
