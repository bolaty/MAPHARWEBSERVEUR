MapharApp.controller("TransPharmacienController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisPharmacien = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT SANTE PHARMACIEN";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransPharmacien", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpTransPharmacien")
      );
    };
    //fin voir plus
    ...
    $scope.transmissionPHAR = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisPharmacien = reponse.data;
          if (
            $scope.elmntTransmisPharmacien[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisPharmacien[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
