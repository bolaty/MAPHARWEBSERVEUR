MapharApp.controller("TransGesaController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisGesa = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT SANTE GESA";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransGesa", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpTransGesa"));
    };
    //fin voir plus
    ...
    $scope.transmissionGESA = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisGesa = reponse.data;
          if (
            $scope.elmntTransmisGesa[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisGesa[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
