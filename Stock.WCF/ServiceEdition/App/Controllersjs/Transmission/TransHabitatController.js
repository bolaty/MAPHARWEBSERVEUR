MapharApp.controller("TransHabitatController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntTransmisHabitat = [];
    $scope.SL_LIBELLEECRAN = "TRANSMISSION CONTRAT HABITAT";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpTransHabitat", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpTransHabitat"));
    };
    //fin voir plus
    ...
    $scope.transmissionHabitat = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntTransmisHabitat = reponse.data;
          if (
            $scope.elmntTransmisHabitat[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntTransmisHabitat[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeTransmission[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
