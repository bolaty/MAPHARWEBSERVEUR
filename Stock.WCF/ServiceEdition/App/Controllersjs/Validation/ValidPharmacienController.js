MapharApp.controller("ValidPharmacienController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValidePharmacien = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT SANTE PHARMACIEN";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidPharmacien", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidPharmacien")
      );
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValidePharmacien = reponse.data;
          if (
            $scope.elmntValidePharmacien[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValidePharmacien[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
