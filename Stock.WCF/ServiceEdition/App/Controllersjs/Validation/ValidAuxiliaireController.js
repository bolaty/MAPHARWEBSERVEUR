MapharApp.controller("ValidAuxiliaireController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideAuxiliaire = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT SANTE AUXILIAIRE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidAuxiliaire", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidAuxiliaire")
      );
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideAuxiliaire = reponse.data;
          if (
            $scope.elmntValideAuxiliaire[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideAuxiliaire[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
