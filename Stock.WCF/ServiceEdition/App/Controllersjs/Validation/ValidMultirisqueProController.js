MapharApp.controller("ValidMultirisqueProController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.objet_envoie = [];
    $scope.listeValidation = [];
    $scope.nombreInfoBulle = "";
    $scope.tableauInfoBulle = "";
    $scope.elmntValideMultirisquePro = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT MULTIRISQUE PROFESSIONNEL";
    ...
    $scope.afficheListeValidation = function (
      periodeDu,
      periodeAu,
      numDePolice,
      numSouscrp,
      ...
      if (
        ...
        $http
          ...
          .then(function (reponse) {
            $scope.$emit("UNLOAD");
            $scope.listeValidation = reponse.data;
            $scope.nombreInfoBulle = $scope.listeValidation.length;
            if (
              $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT === "FALSE"
            ) {
              $scope.$emit("UNLOAD");
              $rootScope.MessageDesListes(
                $scope.listeValidation[0].clsObjetRetour.SL_MESSAGE
              );
            }
          });
      }
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidMultirisquePro", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidMultirisquePro")
      );
    };
    //fin voir plus

    //debut recuper id du contrat a transmettre
    $scope.validation = function (recupId) {
      $scope.idElement = recupId;
    };
    //fin recuper id du contrat a transmettre

    //debut transmettre le contrat
    $scope.validationContrat = function (recupId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideMultirisquePro = reponse.data;
          if (
            $scope.elmntValideMultirisquePro[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideMultirisquePro[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
    ...
    //debut transmettre le contrat
    $scope.AnnulationTransm = function (recuperId) {
      $scope.$emit("LOAD");
      $scope.objet_envoie = [
        {
          CA_CODECONTRAT: recuperId,
          AG_CODEAGENCE: $rootScope.CODE_AGENCE,
          EN_CODEENTREPOT: $rootScope.CODE_ENTREPOT,
          OP_CODEOPERATEUR: $rootScope.CODE_OPERATEUR,
          CA_DATETRANSMISSIONAASSUREUR: "01-01-1900",
      ...
      $http
        .post(
          "/AnnulationTransmission/ListeAnnulationTransmission/",
          $scope.objet_envoie,
          {
            //headers: $scope.headers
          }
        ...
        .then(function (reponse) {
          $scope.annultransVoyage = reponse.data;
          if (
            $scope.annultransVoyage[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.annultransVoyage[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
