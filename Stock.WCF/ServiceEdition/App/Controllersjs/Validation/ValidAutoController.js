MapharApp.controller("ValidAutoController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideAuto = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT AUTO";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidAuto", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpValidAuto"));
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideAuto = reponse.data;
          if ($scope.elmntValideAuto[0].clsObjetRetour.SL_RESULTAT === "TRUE") {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideAuto[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
