MapharApp.controller("ValidHabitatController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideHabitat = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT HABITAT";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidHabitat", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpValidHabitat"));
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideHabitat = reponse.data;
          if (
            $scope.elmntValideHabitat[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideHabitat[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
