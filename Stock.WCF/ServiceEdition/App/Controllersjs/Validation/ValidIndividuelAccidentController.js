MapharApp.controller("ValidIndividuelAccidentController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideIndivAccident = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT INDIVIDUEL ACCIDENT";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidIndivAccident", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidIndivAccident")
      );
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideIndivAccident = reponse.data;
          if (
            $scope.elmntValideIndivAccident[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideIndivAccident[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
