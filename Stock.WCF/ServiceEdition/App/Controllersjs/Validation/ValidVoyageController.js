MapharApp.controller("ValidVoyageController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideVoyage = [];
    $scope.annultransVoyage = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT VOYAGE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidVoyage", JSON.stringify(info));
      $scope.recuperer = JSON.parse(sessionStorage.getItem("vpValidVoyage"));
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideVoyage = reponse.data;
          if (
            $scope.elmntValideVoyage[0].clsObjetRetour.SL_RESULTAT === "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideVoyage[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
