MapharApp.controller("ValidRespoCivileController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideRespoCivile = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT RESPONSABILITE CIVILE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidRespoCivile", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidRespoCivile")
      );
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideRespoCivile = reponse.data;
          if (
            $scope.elmntValideRespoCivile[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideRespoCivile[0].clsObjetRetour.SL_MESSAGE
            );
