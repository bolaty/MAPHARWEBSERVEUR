MapharApp.controller("ValidTransportMarchandiseController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.elmntValideTranspMarch = [];
    $scope.SL_LIBELLEECRAN = "VALIDATION CONTRAT TRANSPORT MARCHANDISE";
    ...
    $scope.voirPlus = function (info) {
      sessionStorage.setItem("vpValidTranspMarch", JSON.stringify(info));
      $scope.recuperer = JSON.parse(
        sessionStorage.getItem("vpValidTranspMarch")
      );
    };
    //fin voir plus
    ...
    $scope.validationContrat = function (recupId) {
      ...
      $http
        ...
        .then(function (reponse) {
          $scope.elmntValideTranspMarch = reponse.data;
          if (
            $scope.elmntValideTranspMarch[0].clsObjetRetour.SL_RESULTAT ===
            "TRUE"
          ) {
            $scope.$emit("UNLOAD");
            $rootScope.TransmissionValidationReussie(
              $scope.elmntValideTranspMarch[0].clsObjetRetour.SL_MESSAGE
            );
            $scope.listeValidation[0].clsObjetRetour.SL_RESULTAT = "FALSE";
          } else {
            $scope.$emit("UNLOAD");
