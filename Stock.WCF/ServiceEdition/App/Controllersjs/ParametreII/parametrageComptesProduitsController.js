MapharApp.controller("parametrageComptesProduitsController", [
  "$scope",
  "$http",
  "$q",
  "$location",
  ...
  function ($scope, $http, $q, $location, $sce, $window, $rootScope) {
    ...
    $scope.Validerparametreoperation = function () {
      $scope.$emit("LOAD");

      $scope.clsCtcontratgaranties = [];
      for (var i = 0; i < $scope.listeretour.length; i++) {
        $scope.objetGarentie = {
          PL_CODENUMCOMPTE: "",
          TO_CODEOPERATIONPARAMETRE: "",
          PL_NUMCOMPTE: "",
          CP_CODEOPERATIONLIBELLECPTE: "",
          SL_LIBELLEECRAN: "Saisie de Continent",
          SL_LIBELLEMOUCHARD: "INSERTIONS",
          TYPEOPERATION: "",
          LG_CODELANGUE: "fr",
        ...
        $scope.objetGarentie.PL_CODENUMCOMPTE =
          $scope.listeretour[i].PL_CODENUMCOMPTE;
        $scope.objetGarentie.TO_CODEOPERATIONPARAMETRE =
          $scope.listeretour[i].TO_CODEOPERATIONPARAMETRE;
        $scope.objetGarentie.PL_NUMCOMPTE = $scope.listeretour[i].PL_NUMCOMPTE;
        $scope.objetGarentie.CP_CODEOPERATIONLIBELLECPTE =
          $scope.listeretour[i].CP_CODEOPERATIONLIBELLECPTE;
      ...
      $http
        .post("/ParametrageComptesProduit/UPDATE/", $scope.objet_envoie, {
          //headers: $scope.headers
        })
        .then(function (reponse) {
          $scope.tabretoursave = reponse.data;
          console.log($scope.tablisteDroitEntrepot);
          if ($scope.tabretoursave[0].clsObjetRetour.SL_RESULTAT == "TRUE") {
            $scope.$emit("UNLOAD");
            $rootScope.EnregistrementReussi(
              $scope.tabretoursave[0].clsObjetRetour.SL_MESSAGE
            );
            setTimeout(function () {
    ...
    $scope.choixNumCompte = function (infosouscr) {
      $scope.formParametrageoperation.numCompte = "";
      $scope.formParametrageoperation.numCompte = infosouscr;
      for (var i = 0; i < $scope.listeretour.length; i++) {
        if (
          $scope.indexligne == $scope.listeretour[i].TO_CODEOPERATIONPARAMETRE
        ) {
          $scope.listeretour[i].PL_NUMCOMPTE =
            $scope.formParametrageoperation.numCompte;
          $scope.veroucompte = true;
          break;
        }
    ...
    $scope.saveparm = function () {
      ...
      if ($scope.FormModifParametre.MontantMin == undefined) {
        $scope.FormModifParametre.MontantMin = 0;
      }
      if ($scope.FormModifParametre.MontantMax == undefined) {
        $scope.FormModifParametre.MontantMax = 0;
      }
      if ($scope.FormModifParametre.Montant == undefined) {
        $scope.FormModifParametre.Montant = 0;
      }
      if ($scope.FormModifParametre.valeur == undefined) {
        $scope.FormModifParametre.valeur = "";
      }
      if ($scope.FormModifParametre.Taux == undefined) {
        $scope.FormModifParametre.Taux = 0;
      }
      if ($scope.FormModifParametre.numCompte == undefined) {
        $scope.FormModifParametre.numCompte = 0;
      }
      if ($scope.FormModifParametre.code == undefined) {
        $scope.FormModifParametre.code = "";
      }
      if ($scope.FormModifParametre.libelle == undefined) {
    ...
    $scope.selectval = function (index, infoliste) {
      // marquer la ligne selectionnée
      for (var i = 0; i < $scope.listeretour.length; i++) {
        if (i != index) {
          $("#idTable" + i).css("background-color", "white");
        } else {
          $("#idTable" + index).css("background-color", "beige");
        }
      }

      $scope.formParametrageoperation.numCompte = "";
      $scope.formParametrageoperation.Libelle = infoliste.CP_LIBELLE;
      $scope.indexligne = infoliste.TO_CODEOPERATIONPARAMETRE;
      $scope.veroucompte = false;
    };

    // --------------- fin DEV bolaty ------------------------

    //---------------- Paramétrage des autres écritures --------------------------------------------//
    ...
    //---------------- fin Paramétrage des autres écritures --------------------------------------------//
  },
]);
