            ...
            try
            {
                ...
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in DataSet.Tables[0].Rows)
                    {
                        ...
                        clsEditionEtatCaisse.PL_MONTANTPERIODECREDITENCOURSGESTION = (row["PL_MONTANTPERIODECREDITENCOURSGESTION"].ToString() != "") ? double.Parse(row["PL_MONTANTPERIODECREDITENCOURSGESTION"].ToString()) : 0;//row["PL_MONTANTPERIODECREDITENCOURSGESTION"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTSOLDEFINALDEBITGESTION = (row["PL_MONTANTSOLDEFINALDEBITGESTION"].ToString() != "") ? double.Parse(row["PL_MONTANTSOLDEFINALDEBITGESTION"].ToString()) : 0;//row["PL_MONTANTSOLDEFINALDEBITGESTION"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTSOLDEFINALCREDITGESTION = (row["PL_MONTANTSOLDEFINALCREDITGESTION"].ToString() != "") ? double.Parse(row["PL_MONTANTSOLDEFINALCREDITGESTION"].ToString()) : 0;//row["PL_MONTANTSOLDEFINALCREDITGESTION"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTPERIODEPRECEDENTDEBITBILAN = (row["PL_MONTANTPERIODEPRECEDENTDEBITBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTPERIODEPRECEDENTDEBITBILAN"].ToString()) : 0;//row["PL_MONTANTPERIODEPRECEDENTDEBITBILAN"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTPERIODEPRECEDENTCREDITBILAN = (row["PL_MONTANTPERIODEPRECEDENTCREDITBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTPERIODEPRECEDENTCREDITBILAN"].ToString()) : 0;//row["PL_MONTANTPERIODEPRECEDENTCREDITBILAN"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTPERIODEDEBITENCOURSBILAN = (row["PL_MONTANTPERIODEDEBITENCOURSBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTPERIODEDEBITENCOURSBILAN"].ToString()) : 0;//row["PL_MONTANTPERIODEDEBITENCOURSBILAN"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTPERIODECREDITENCOURSBILAN = (row["PL_MONTANTPERIODECREDITENCOURSBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTPERIODECREDITENCOURSBILAN"].ToString()) : 0;//row["PL_MONTANTPERIODECREDITENCOURSBILAN"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTSOLDEFINALDEBITBILAN = (row["PL_MONTANTSOLDEFINALDEBITBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTSOLDEFINALDEBITBILAN"].ToString()) : 0;//row["PL_MONTANTSOLDEFINALDEBITBILAN"].ToString();
                        clsEditionEtatCaisse.PL_MONTANTSOLDEFINALCREDITBILAN = (row["PL_MONTANTSOLDEFINALCREDITBILAN"].ToString() != "") ? double.Parse(row["PL_MONTANTSOLDEFINALCREDITBILAN"].ToString()) : 0;//row["PL_MONTANTSOLDEFINALCREDITBILAN"].ToString();
                        clsEditionEtatCaisse.PL_TOTALCHARGEFINPERIODE = (row["PL_TOTALCHARGEFINPERIODE"].ToString() != "") ? double.Parse(row["PL_TOTALCHARGEFINPERIODE"].ToString()) : 0;//row["PL_TOTALCHARGEFINPERIODE"].ToString();
                        clsEditionEtatCaisse.PL_TOTALPRODUITFINPERIODE = (row["PL_TOTALPRODUITFINPERIODE"].ToString() != "") ? double.Parse(row["PL_TOTALPRODUITFINPERIODE"].ToString()) : 0;//row["PL_TOTALPRODUITFINPERIODE"].ToString();
                        clsEditionEtatCaisse.PL_RESULTATNETPROVISOIRE = (row["PL_RESULTATNETPROVISOIRE"].ToString() != "") ? double.Parse(row["PL_RESULTATNETPROVISOIRE"].ToString()) : 0;//row["PL_RESULTATNETPROVISOIRE"].ToString();
                        clsEditionEtatCaisse.PL_TOTALCOMPTEGESTION = (row["PL_TOTALCOMPTEGESTION"].ToString() != "") ? double.Parse(row["PL_TOTALCOMPTEGESTION"].ToString()) : 0;//row["PL_TOTALCOMPTEGESTION"].ToString();
                        clsEditionEtatCaisse.PL_TOTALCOMPTEBILAN = (row["PL_TOTALCOMPTEBILAN"].ToString() != "") ? double.Parse(row["PL_TOTALCOMPTEBILAN"].ToString()) : 0;//row["PL_TOTALCOMPTEBILAN"].ToString();
            ...
            try
            {
                ...
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in DataSet.Tables[0].Rows)
                    {
                        ...
                        clsEditionEtatCaisse.MV_NOMTIERS = row["MV_NOMTIERS"].ToString();
                        clsEditionEtatCaisse.NUMEROBORDEREAU = row["NUMEROBORDEREAU"].ToString();
                        clsEditionEtatCaisse.MV_SOLDEPRECEDENT = (row["MV_SOLDEPRECEDENT"].ToString() != "") ? double.Parse(row["MV_SOLDEPRECEDENT"].ToString()) : 0;//row["MV_SOLDEPRECEDENT"].ToString();
                        clsEditionEtatCaisse.MV_NUMSEQUENCE = row["MV_NUMSEQUENCE"].ToString();
                        clsEditionEtatCaisse.MV_SOLDE = (row["MV_SOLDE"].ToString() != "") ? double.Parse(row["MV_SOLDE"].ToString()) : 0;//row["MV_SOLDE"].ToString();
                        clsEditionEtatCaisse.NUMEROBORDEREAU1 = row["NUMEROBORDEREAU1"].ToString();
                        clsEditionEtatCaisse.MV_SOLDEFINAL = (row["MV_SOLDEFINAL"].ToString() != "") ? double.Parse(row["MV_SOLDEFINAL"].ToString()) : 0;//row["MV_SOLDEFINAL"].ToString();
                        clsEditionEtatCaisse.MS_NUMSEQUENCE = row["MS_NUMSEQUENCE"].ToString();
                        clsEditionEtatCaisse.PL_CODENUMCOMPTE = row["PL_CODENUMCOMPTE"].ToString();
                        clsEditionEtatCaisse.MC_SOLDEPRECEDENT = (row["MC_SOLDEPRECEDENT"].ToString() != "") ? double.Parse(row["MC_SOLDEPRECEDENT"].ToString()) : 0;//row["MC_SOLDEPRECEDENT"].ToString();
                        clsEditionEtatCaisse.TI_NUMTIERS = row["TI_NUMTIERS"].ToString();
                        ...
                        clsEditionEtatCaisse.OP_NOMPRENOM = row["OP_NOMPRENOM"].ToString();
                        clsEditionEtatCaisse.MR_LIBELLE = row["MR_LIBELLE"].ToString();
                        clsEditionEtatCaisse.TOTALCAISSE = (row["TOTALCAISSE"].ToString() != "") ? double.Parse(row["TOTALCAISSE"].ToString()) : 0;//row["TOTALCAISSE"].ToString();
                        clsEditionEtatCaisse.TOTALBANQUE = (row["TOTALBANQUE"].ToString() != "") ? double.Parse(row["TOTALBANQUE"].ToString()) : 0;//row["TOTALBANQUE"].ToString();
                        clsEditionEtatCaisse.clsObjetRetour = new Common.clsObjetRetour();
                        clsEditionEtatCaisse.clsObjetRetour.SL_CODEMESSAGE = "00";
                        clsEditionEtatCaisse.clsObjetRetour.SL_RESULTAT = "TRUE";
                        clsEditionEtatCaisse.clsObjetRetour.SL_MESSAGE = "Opération réalisée avec succès !!!";
