...
namespace Stock.WCF
{
    ...
    public partial class wsEditionEtatStock : IwsEditionEtatStock
    {
            ...
            try
            {
                ...
                if (DataSet.Tables[0].Rows.Count > 0)
                {
                    foreach (DataRow row in DataSet.Tables[0].Rows)
                    {
                        ...
                        clsEditionEtatStock.MK_MONTANT = (row["MK_MONTANT"].ToString() != "") ? double.Parse(row["MK_MONTANT"].ToString()) : 0;

                        clsEditionEtatStock.MK_MONTANTTOTALREMISE = (row["MK_MONTANTTOTALREMISE"].ToString() != "") ? double.Parse(row["MK_MONTANTTOTALREMISE"].ToString()) : 0;
                        clsEditionEtatStock.MK_MONTANTTRANSPORT = (row["MK_MONTANTTRANSPORT"].ToString() != "") ? double.Parse(row["MK_MONTANTTRANSPORT"].ToString()) : 0;
                        clsEditionEtatStock.MK_MONTANTVERSE = (row["MK_MONTANTVERSE"].ToString() != "") ? double.Parse(row["MK_MONTANTVERSE"].ToString()) : 0;
                        ...
                        clsEditionEtatStock.MD_QUANTITESORTIE = (row["MD_QUANTITESORTIE"].ToString() != "") ? double.Parse(row["MD_QUANTITESORTIE"].ToString()) : 0;
                        clsEditionEtatStock.LV_QUANTITELIVRER = (row["LV_QUANTITELIVRER"].ToString() != "") ? double.Parse(row["LV_QUANTITELIVRER"].ToString()): 0;
        ...
        ///<returns>Une collection de clsInput valeur du résultat de la requete</returns>
        ///<author>Home Technology</author>
        public List<HT_Stock.BOJ.clsEditionEtatStock> pvgMiseAJourZeCaisse(List<HT_Stock.BOJ.clsEditionEtatStock> Objet)
        {
            ...
            {
                //--TEST DES CHAMPS OBLIGATOIRES
                clsEditionEtatStocks = TestChampObligatoireListeGenerer(Objet[Idx]);
                //--VERIFICATION DU RESULTAT DU TEST
                if (clsEditionEtatStocks[0].clsObjetRetour.SL_RESULTAT == "FALSE") return clsEditionEtatStocks;
                //--TEST CONTRAINTE
                clsEditionEtatStocks = TestTestContrainteListe(Objet[Idx]);
            ...
            try
            {
                ...
                foreach (HT_Stock.BOJ.clsEditionEtatStock clsEditionEtatStockDTO in Objet)
                {
                    ...
                    //"@AG_CODEAGENCE", "@EN_CODEENTREPOT", "@DATEJOURNEE1", "@DATEJOURNEE2", "@TP_CODETYPETIERS", "@ET_TYPEETAT", "@ET_TYPELISTE", "@OP_CODEOPERATEUREDITION", "@GP_CODEGROUPE", "@PY_CODEPAYS", "@NT_CODENATURETIERS", "@CODEDECRYPTAGE"
                    clsObjetEnvoi.OE_PARAM = new string[] { clsEditionEtatStockDTO.AG_CODEAGENCE, clsEditionEtatStockDTO.DATEDEBUT, clsEditionEtatStockDTO.DATEFIN, clsEditionEtatStockDTO.ZC_CODE };
                    clsObjetEnvoi.OE_A = clsEditionEtatStockDTO.clsObjetEnvoi.OE_A;
                    clsObjetEnvoi.OE_Y = clsEditionEtatStockDTO.clsObjetEnvoi.OE_Y;
...
//DataSet = 
                clsEditionEtatStockWSBLL.pvgMiseAJourZeCaisse(clsDonnee, clsObjetEnvoi);
                clsEditionEtatStocks = new List<HT_Stock.BOJ.clsEditionEtatStock>();
