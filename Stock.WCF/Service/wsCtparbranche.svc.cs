...
namespace Stock.WCF
{
    ...
    ///<returns>Une collection de clsInput valeur du résultat de la requete</returns>
    ///<author>Home Technology</author>
    public List<HT_Stock.BOJ.clsCtparbranche> pvgChargerDansDataSetPourComboSelonRisque(List<HT_Stock.BOJ.clsCtparbranche> Objet)
        {
    ...
    {
        clsDonnee.pvgConnectionBase();
        DataSet = clsCtparbrancheWSBLL.pvgChargerDansDataSetPourComboSelonRisque(clsDonnee, clsObjetEnvoi);
        clsCtparbranches = new List<HT_Stock.BOJ.clsCtparbranche>();
        if (DataSet.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow row in DataSet.Tables[0].Rows)
