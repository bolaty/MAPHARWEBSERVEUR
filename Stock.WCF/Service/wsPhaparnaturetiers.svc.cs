...
namespace Stock.WCF
{
    ...
    public partial class wsPhaparnaturetiers : IwsPhaparnaturetiers
    {
        ...
        ///<returns>Une collection de clsInput valeur du résultat de la requete</returns>
        ///<author>Home Technology</author>
        public List<HT_Stock.BOJ.clsPhaparnaturetiers> pvgChargerDansDataSetPourComboEdition(List<HT_Stock.BOJ.clsPhaparnaturetiers> Objet)
            {
        ...
        clsDonnee.vogUtilisateur = clsObjetEnvoi.OE_X;
            ...
            {
                //--TEST DES CHAMPS OBLIGATOIRES
                clsPhaparnaturetierss = TestChampObligatoireListePourEdition(Objet[Idx]);
                //--VERIFICATION DU RESULTAT DU TEST
                if (clsPhaparnaturetierss[0].clsObjetRetour.SL_RESULTAT == "FALSE") return clsPhaparnaturetierss;
                //--TEST CONTRAINTE
                clsPhaparnaturetierss = TestTestContrainteListe(Objet[Idx]);
        ...
        {
            clsDonnee.pvgConnectionBase();
            DataSet = clsPhaparnaturetiersWSBLL.pvgChargerDansDataSetPourComboEdition(clsDonnee, clsObjetEnvoi);
            clsPhaparnaturetierss = new List<HT_Stock.BOJ.clsPhaparnaturetiers>();
            if (DataSet.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in DataSet.Tables[0].Rows)
