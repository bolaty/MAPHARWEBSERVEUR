...
namespace Stock.WCF
{
    // REMARQUE : vous pouvez utiliser la commande Renommer du menu Refactoriser pour changer le nom de classe "wsNumeroDebut" à la fois dans le code, le fichier svc et le fichier de configuration.
    // REMARQUE : pour lancer le client test WCF afin de tester ce service, sélectionnez wsNumeroDebut.svc ou wsNumeroDebut.svc.cs dans l'Explorateur de solutions et démarrez le débogage.
    public partial class wsNumeroDebut : IwsNumeroDebut
    {
        ...
        try
        {
            ...
            if (DataSet.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in DataSet.Tables[0].Rows)
                {
                    ...
                    clsPlancomptable.PL_CODENUMERODEBUT = row["PL_CODENUMERODEBUT"].ToString();
                    clsPlancomptable.PL_LIBELLENUMERODEBUT = row["PL_LIBELLENUMERODEBUT"].ToString();
