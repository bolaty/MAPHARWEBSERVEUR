...
namespace Stock.WSTOOLS
{
    public class clsChaineCaractere
    {
        ...
        public bool pvgTestLibelle(string vppValeurSaisie)
        {
         ...
         public int pvgCodeJournal(string vppModeReglement)
        {


            int vlpJO_CODEJOURNAL = 0;

            //-2-1-REGLEMENT EN ESPECE : LE COMPTE CAISSE EST DEBITE
            if (vppModeReglement == "01")
            {
                // caisse
                vlpJO_CODEJOURNAL = 6;
            }
            if (vppModeReglement == "02" || vppModeReglement == "04")
            {
                //banque
                vlpJO_CODEJOURNAL = 2;
            }
            if (vppModeReglement == "03" || vppModeReglement == "05" || vppModeReglement == "06" || vppModeReglement == "07")
            {
                //od
                vlpJO_CODEJOURNAL = 3;
            }
